...
def reset_idx(env, env_ids: torch.Tensor | None):
    ...
    workspace_center = torch.tensor(env.cfg_task.hole_workspace_xy_center, device=env.device).unsqueeze(0).repeat(n, 1)
    workspace_half_range = torch.tensor(env.cfg_task.hole_workspace_xy_half_range, device=env.device).unsqueeze(0).repeat(n, 1)
    hole_xy_local = workspace_center + (2 * torch.rand((n, 2), device=env.device) - 1) * workspace_half_range

    for _ in range(32):
        init_xy_dist = torch.norm(peg_tip_w[:, 0:2] - (hole_xy_local + env.scene.env_origins[env_ids, 0:2]), p=2, dim=-1)
        invalid = init_xy_dist > env.cfg_task.max_initial_xy_dist
        if not torch.any(invalid):
            break
        resampled_xy = workspace_center[invalid] + (2 * torch.rand((int(invalid.sum()), 2), device=env.device) - 1) * workspace_half_range[invalid]
        hole_xy_local[invalid] = resampled_xy

    hole_state = env._hole.data.default_root_state[env_ids].clone()
    hole_state[:, 0:2] = hole_xy_local + env.scene.env_origins[env_ids, 0:2]
    hole_state[:, 2] = env.cfg_task.hole.top_z + env.scene.env_origins[env_ids, 2]
